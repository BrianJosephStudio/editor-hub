...
export const getProjectFolderObjectByDate = async () => {
  try {
    ...
    return baseProjectFolders.entries.find((entry: any) => entry.name.includes(targetFolderDataName))
...
export const createBaseProjectFolderByDate = async (projectData: ProjectData) => {
  try {
    const targetFoldernameDate = moment().format("MMMM YYYY")
    const projectId = `${projectData.projectType === ProjectType.SHORT ? "S" : ""}${projectData.projectNumber}`
